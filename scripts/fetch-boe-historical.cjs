// scripts/fetch-boe-historical.cjs
// Script de descarga histórica del BOE con claves API exclusivas
...
if (!/^\d{8}$/.test(fecha)) {
  console.error("❌ Usa: npm run fetch-boe-historical YYYYMMDD")
  process.exit(1)
}
...
async function fetchBOEHistorical(fecha) {
  // 0) Si ya existe xmlDir con algo → parar
  try {
    const files = await fs.readdir(xmlDir)
    if (files.length) {
      console.log(`🟡 [HISTÓRICO] Ya existen XMLs para ${fecha}, no hago nada.`)
      return
    }
  ...
  console.log(`🔎 [HISTÓRICO] Descargando sumario JSON de ${fecha}...`)
  const sumRes = await fetch(sumUrl, { headers:{ Accept:'application/json' } })
  if (!sumRes.ok) {
    console.error(`❌ [HISTÓRICO] Error ${sumRes.status} al descargar sumario`)
    return
  }
  ...
  console.log(`⬇️  [HISTÓRICO] Descargando ${allItems.length} disposiciones...`)
  ...
  console.log(`✅ [HISTÓRICO] XMLs en ${xmlDir}  y  JSONs en ${jsonDir}`)

  // Clasificación automática con Groq (versión histórica)
  ...
  try {
    ...
    console.log('💡 [HISTÓRICO] Los datos JSON están disponibles, puedes clasificar manualmente más tarde')
  }
}

fetchBOEHistorical(fecha).catch(err => {
  console.error(err)
  process.exit(1)
})
